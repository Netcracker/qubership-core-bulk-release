...
//            List<GoRepositoryInfo> repositoryInfoList = repositories.stream()
//                    .map(rc -> {
//                        try {
//                            PipedOutputStream o = new PipedOutputStream();
//                            PipedInputStream pipedInputStream = new PipedInputStream(o, 16384);
//                            Future<GoRepositoryInfo> future = executorService.submit(() ->
...
//            List<GoRepositoryInfo> resolvedSupportRepositories = resolveSupportBranches(repositoryInfoList,
//                    baseDir, gitConfig, mavenConfig, createMissingBranches, validateSameVersionUpToLevel,
//                    skipValidationForGAPatterns, out);
//
//            Graph<String, StringEdge> graph = new SimpleDirectedGraph<>(StringEdge.class);
//
//            for (GoRepositoryInfo repositoryInfo : resolvedSupportRepositories) {
//                graph.addVertex(repositoryInfo.getUrl());
//            }
...
//                        .forEach(ri -> graph.addEdge(ri.getUrl(), repositoryInfo.getUrl()));
//            }
//            List<GoRepositoryInfo> independentRepos = resolvedSupportRepositories.stream()
//                    .filter(ri -> graph.incomingEdgesOf(ri.getUrl()).isEmpty()).toList();
//            List<GoRepositoryInfo> dependentRepos = resolvedSupportRepositories.stream()
//                    .filter(ri -> !graph.incomingEdgesOf(ri.getUrl()).isEmpty()).collect(Collectors.toList());
//            Map<Integer, List<GoRepositoryInfo>> groupedReposMap = new TreeMap<>();
//            groupedReposMap.put(0, independentRepos);
//            int level = 1;
//            while (!dependentRepos.isEmpty()) {
//                List<GoRepositoryInfo> prevLevelRepos = IntStream.range(0, level).boxed().flatMap(lvl -> groupedReposMap.get(lvl).stream()).toList();
//                List<GoRepositoryInfo> thisLevelRepos = dependentRepos.stream()
//                        .filter(ri -> graph.incomingEdgesOf(ri.getUrl()).stream().map(StringEdge::getSource)
//                                .allMatch(dependentRepoUrl -> prevLevelRepos.stream().map(GoRepositoryInfo::getUrl)
//                                        .collect(Collectors.toSet()).contains(dependentRepoUrl))).toList();
//                groupedReposMap.put(level, thisLevelRepos);
//                dependentRepos.removeAll(thisLevelRepos);
//                level++;
...
//    }
//
//    List<GoRepositoryInfo> resolveSupportBranches(Collection<GoRepositoryInfo> repositories,
//                                                  String baseDir, GitConfig gitConfig,
//                                                  MavenConfig mavenConfig, boolean createMissingBranches,
//                                                  VersionIncrementType validateSameVersionUpToLevel,
//                                                  List<Pattern> skipValidationForGAPatterns,
//                                                  OutputStream out) {
//        Map<String, GoRepositoryInfo> processedVersionedRepositoryTree = new LinkedHashMap<>();
//        // 1. find repositories with a non-empty version param
//        List<GoRepositoryInfo> versionedRepositories = repositories.stream().filter(rc -> rc.getVersion() != null).toList();
...
//
//        List<GoRepositoryInfo> updatedRepositories = repositories.stream()
...
//                .filter(ri -> {
//                    List<GoRepositoryInfo> parents = linker.getRepositoriesUsingThis(ri);
//                    List<GoRepositoryInfo> children = linker.getRepositoriesUsedByThis(ri);
//                    List<GoRepositoryInfo> resolve = Stream.concat(parents.stream(), children.stream())
...
//    }
//
//    void resolveSupportBranches(Map<String, GoRepositoryInfo> processedVersionedRepositoryTree,
//                                Map<GoRepositoryInfo, DependencyType> parentsStack, RepositoryConfig fromRepository,
//                                Collection<GoRepositoryInfo> repositories,
...
//        GoRepositoryInfo repositoryInfo = this.createRepositoryInfo(baseDir, gitConfig, repositoryConfig, out);
//        if (processedVersionedRepositoryTree.get(fromRepository.getUrl()) instanceof GoRepositoryInfo processedRepositoryInfo) {
...
//        Map<GAV, GoRepositoryInfo> conflicts = repositoryInfo.getModuleDependencies().stream()
...
//                                        return Optional.<GoRepositoryInfo>empty();
//                                    }
...
//                            .sorted(Comparator.<Map.Entry<GAV, GoRepositoryInfo>, String>comparing(e -> e.getKey().getVersion())
...
//    }
//
//    void resolveDependent(Map<String, GoRepositoryInfo> processedVersionedRepositoryTree,
//                          Map<GoRepositoryInfo, DependencyType> parentsStack,
//                          GoRepositoryInfo repositoryInfo,
//                          Collection<GoRepositoryInfo> repositories,
//                          String baseDir, GitConfig gitConfig, MavenConfig mavenConfig, boolean createMissingBranches,
//                          VersionIncrementType validateSameVersionUpToLevel,
//                          List<Pattern> skipValidationForGAPatterns,
//                          OutputStream out) {
//        List<GoRepositoryInfo> updatedRepositories = repositories.stream().map(ri -> {
...
//        }).toList();
//        RepositoryInfoLinker linker = new RepositoryInfoLinker(updatedRepositories);
//        List<GoRepositoryInfo> children = linker.getRepositoriesUsedByThis(repositoryInfo);
...
//
//            Map<GoRepositoryInfo, DependencyType> stack = new LinkedHashMap<>(parentsStack);
...
//        });
//        List<GoRepositoryInfo> parents = linker.getRepositoriesUsingThis(repositoryInfo);
...
//            if (version == null) {
//                version = findParentRepositoryVersion(baseDir, gitConfig, parent, gavs, out);
//            }
...
//                    GoRepositoryInfo repositoryInfo = createRepositoryInfo(baseDir, gitConf, repositoryConfig, out);
...
//    }
//
//    void preparePOMsForDev(String baseDir, GoRepositoryInfo repositoryInfo, MavenConfig mavenConfig,
...
//        GoRepositoryInfo repositoryInfo = this.createRepositoryInfo(baseDir, gitConf, repositoryConfig, out);
...
//    }
//
//    GoRepositoryInfo createRepositoryInfo(String baseDir, GitConfig gitConf, RepositoryConfig repositoryConfig, OutputStream out) {
